...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-serif text-2xl font-bold text-gray-800">➕ Tambah Kartu Keluarga</h1>
            <p class="text-xs text-gray-500 mt-1">Masukkan data No. KK, Nama Kepala Keluarga, dan Alamat.</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-xs p-6 sm:p-8">
        <form action="{{ route('admin.families.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="kk_number" id="kk_number" value="{{ old('kk_number') }}" maxlength="16" required
                    placeholder="Contoh: 3309123456780001"
            ...
            <div>
                ...
                <input type="text" name="head_of_family" id="head_of_family" value="{{ old('head_of_family') }}" required
            ...
            <div>
                ...
                <textarea name="address" id="address" rows="3" required
                    placeholder="RT / RW, Dukuh, Desa Catur, Sambi, Boyolali"
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
                @error('address')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
