...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-serif text-2xl font-bold text-gray-800">✏️ Edit Template Surat</h1>
            <p class="text-xs text-gray-500 mt-1">Perbarui format template {{ $template->title ?? $template->name }}.</p>
        </div>
        ...
        <form action="{{ route('admin.letter-templates.update', $template->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="2"
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm @error('description') border-red-500 @enderror">{{ old('description', $template->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="template_text" id="template_text" rows="12" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-xs font-mono bg-gray-50 @error('template_text') border-red-500 @enderror">{{ old('template_text', $template->template_text) }}</textarea>
                @error('template_text')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
