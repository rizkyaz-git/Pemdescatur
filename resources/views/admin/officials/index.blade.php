...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-1">
        <div>
            ...
            <p class="text-xs text-[#64748B] mt-1 font-medium">Kelola susunan aparatur desa dan urutkan perangkat sesuai struktur.</p>
        </div>
        <div class="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <button type="button" 
                    @click="saveOrder()" 
                    :disabled="isSaving"
                    class="inline-flex items-center gap-1.5 bg-white hover:bg-[#F4F6F5] text-[#0F4C3A] hover:text-[#072C21] border border-[#DDE5E1] text-xs font-semibold px-3.5 py-2.5 rounded-lg shadow-2xs transition cursor-pointer disabled:opacity-60"
                    title="Simpan susunan urutan perangkat desa">
                <svg class="w-3.5 h-3.5 text-[#0F4C3A]" :class="{ 'animate-spin': isSaving }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('public.officials') }}" target="_blank" 
               class="inline-flex items-center gap-1.5 bg-white hover:bg-[#F4F6F5] text-[#607089] hover:text-[#10233F] border border-[#DDE5E1] text-xs font-semibold px-3 py-2.5 rounded-lg shadow-2xs transition cursor-pointer"
    ...
    <!-- 2. Data Table Container (Standard Admin Table: Radius 12px, Border #DDE5E1) -->
            ...
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-[#F7F9F8] text-[#607089] text-[11px] font-bold uppercase tracking-wider border-b border-[#DDE5E1]">
                        <th class="w-20 px-3.5 py-3 text-center">Urutan</th>
                ...
                <tbody id="sortable-officials" class="divide-y divide-[#F1F5F9] text-slate-700 bg-white">
                    @forelse($officials as $off)
                        <tr data-id="{{ $off->id }}" class="hover:bg-[#F7F9F8] transition-colors group">
                            <td class="w-20 px-3.5 py-3 text-center cursor-grab active:cursor-grabbing text-slate-400 group-hover:text-slate-600 drag-handle" title="Tarik baris ini untuk mengatur urutan">
                                <div class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-slate-100 group-hover:bg-[#E6F4EE] border border-slate-200/80 group-hover:border-emerald-200 text-slate-700 group-hover:text-[#0F4C3A] font-bold text-xs select-none transition-colors">
                            ...
                            <td class="px-4 py-3">
                                @if($off->photo_path)
                                    <img src="{{ asset('storage/' . $off->photo_path) }}" class="w-9 h-9 rounded-full object-cover border border-[#DDE5E1]">
                                @else
                                    <div class="w-9 h-9 rounded-full bg-[#E6F4EE] text-[#0F4C3A] font-bold text-xs flex items-center justify-center border border-emerald-200">
                                        {{ strtoupper(substr($off->name, 0, 2)) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-4 py-3 font-semibold text-[#10233F]">
                                {{ $off->name }}
                            </td>
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-semibold rounded-md bg-amber-50 text-amber-800 border border-amber-200/70">
                                    {{ $off->position }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center">
                                ...
                                <p class="text-xs font-semibold text-[#10233F]">Belum ada data perangkat desa</p>
                                <p class="text-[11px] text-[#607089] mt-0.5">Tambahkan aparatur desa pertama melalui tombol di atas.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
