...
@section('content')
    <div class="space-y-6">
        ...
        @if(session('success'))
            <div
                class="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl flex items-center justify-between text-sm shadow-xs">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @foreach($pojoks as $index => $pojok)
                ...
                @endphp
                <div
                    class="bg-white rounded-[20px] border border-[#E2E8F0] overflow-hidden shadow-xs hover:shadow-md hover:border-[#0F4C3A]/30 transition-all flex flex-col justify-between group">
                    <div>
                        <!-- Photo Header Container -->
                        <div class="relative w-full aspect-[16/9] bg-slate-100 overflow-hidden">
                            <img src="{{ $coverPhoto }}" alt="{{ $pojok->nama }}"
                                class="w-full h-full object-cover group-hover:scale-105 transition duration-500">

                            <!-- Badges on Photo -->
                            <div class="absolute top-3 left-3 flex items-center gap-1.5">
                                <span
                                    class="px-2.5 py-1 rounded-full text-[10px] font-bold bg-white/90 backdrop-blur-md text-slate-800 shadow-xs border border-white/40 uppercase tracking-wider">
                                    Pilar 0{{ $index + 1 }}
                                </span>
                                @if($hasCustomPhoto)
                                ...
                                @else
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-medium bg-slate-900/75 backdrop-blur-md text-slate-200 shadow-xs">
                                        Foto Bawaan
                                    </span>
                                @endif
                            </div>
                                ...
                                @if($pojok->kurikulums->isNotEmpty())
                                    <div class="space-y-1.5">
                                        @foreach($pojok->kurikulums->take(2) as $file)
                                            <div
                                                class="flex items-center gap-2 p-2 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] text-xs">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    ...
                                                    <span
                                                        class="text-[10px] text-slate-400 block">{{ $file->formatted_file_size }}</span>
                                                </div>
                                            </div>
                                        @endforeach
                                        @if($pojok->kurikulums->count() > 2)
                                            <span
                                                class="text-[11px] text-slate-400 font-medium block pl-1">+{{ $pojok->kurikulums->count() - 2 }}
                                                file lainnya</span>
                                        @endif
                                    </div>
                                @else
                    ...
                    <div class="p-5 pt-0">
                        <a href="{{ route('admin.ppko.edit', $pojok) }}"
                            class="w-full flex items-center justify-center gap-2 bg-[#0F4C3A] hover:bg-[#072C21] text-white font-semibold text-xs py-2.5 px-4 rounded-xl transition shadow-xs">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
