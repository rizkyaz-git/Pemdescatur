...
<div class="max-w-3xl mx-auto space-y-6 font-sans">
    ...
    <div class="flex items-center gap-3.5">
        ...
        <h1 class="font-jakarta text-2xl font-bold text-[#0F172A]">Edit Data Pengguna</h1>
    </div>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select name="role" id="role" required
                    ...
                    <option value="admin_pemdes" {{ old('role', $user->role) === 'admin_pemdes' || old('role', $user->role) === 'admin_modul' ? 'selected' : '' }}>
                        Admin Pemdes — Akses profil desa, berita, perangkat desa, galeri & layanan publik
                    </option>
                    <option value="ppk_ormawa" {{ old('role', $user->role) === 'ppk_ormawa' || old('role', $user->role) === 'admin_ppp_ormawa' ? 'selected' : '' }}>
            ...
            <div class="pt-4 border-t border-[#F1F5F9] space-y-4">
                <div>
                    <h3 class="font-jakarta font-bold text-sm text-[#0F172A]">Ganti Kata Sandi (Opsional)</h3>
                    <p class="text-xs text-[#64748B]">Biarkan kosong jika Anda tidak ingin mengganti kata sandi pengguna ini.</p>
                </div>
