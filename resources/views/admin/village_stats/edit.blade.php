...
@section('title', 'Edit Data Statistik Infografis')
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-xs">
        <div class="flex items-center gap-3 mb-5">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Perbarui data untuk chart infografis profil desa</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.village-stats.update', $villageStat->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="category_id" required class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A3D29]">
                    ...
                    <option value="{{ $cat->id }}" {{ (old('category_id', $villageStat->category_id) == $cat->id) ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="label" value="{{ old('label', $villageStat->label) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="value" value="{{ old('value', $villageStat->value) }}" required step="0.01"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A3D29]">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="year" value="{{ old('year', $villageStat->year) }}" placeholder="kosongkan jika tidak ada tahun"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A3D29]">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 mb-1">Sub Grup</label>
                    <input type="text" name="sub_group" value="{{ old('sub_group', $villageStat->sub_group) }}" placeholder="contoh: Baik, Sedang, Rusak"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A3D29]">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $villageStat->order) }}" min="0"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A3D29]">
            </div>
