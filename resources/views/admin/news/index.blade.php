                            ...
                            <tr class="hover:bg-[#F8FAFC]/80 transition-colors">
                                ...
                                <td class="px-4 py-3.5 align-middle text-center whitespace-nowrap">
                                    <div class="inline-flex items-center justify-center gap-1.5">
                                        <!-- Tombol Set Berita Utama -->
                                        <form action="{{ route('admin.news.set-featured', $news->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            <button type="submit"
                                                title="{{ (string) $featuredNewsId === (string) $news->id ? 'Batalkan sebagai Berita Utama' : 'Jadikan Berita Utama' }}"
                                                class="inline-flex items-center gap-1 h-7 px-2.5 rounded-lg border text-xs font-semibold transition shadow-2xs cursor-pointer {{ (string) $featuredNewsId === (string) $news->id ? 'bg-amber-50 border-amber-300 text-amber-800 hover:bg-amber-100' : 'border-[#E2E8F0] text-slate-600 hover:text-amber-700 hover:border-amber-300 hover:bg-amber-50/50' }}">
                                                ...
                                                <span>{{ (string) $featuredNewsId === (string) $news->id ? 'Utama' : 'Set Utama' }}</span>
                                            </button>
                                        </form>
