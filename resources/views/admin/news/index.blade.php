...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-[20px] border border-[#E2E8F0] shadow-xs">
        <div>
            ...
            <h1 class="font-jakarta text-2xl font-bold text-[#0F172A]">Daftar Berita & Pengumuman</h1>
            <p class="text-xs text-[#64748B] mt-1">Kelola publikasi artikel kegiatan, agenda pemerintahan, dan pengumuman resmi Desa Catur.</p>
        </div>
        <a href="{{ route('admin.news.create') }}" class="inline-flex items-center gap-2 bg-[#0F4C3A] hover:bg-[#072C21] text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-xs transition shrink-0">
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-[#F8FAFC] text-[#64748B] text-[11px] font-bold uppercase tracking-wider border-b border-[#E2E8F0]">
                        ...
                        <th class="px-5 py-3.5">Judul Warta</th>
                ...
                <tbody class="divide-y divide-[#F1F5F9]">
                    @forelse($newsList as $news)
                        <tr class="hover:bg-[#F8FAFC]/80 transition-colors">
                            <td class="px-5 py-4">
                                @if($news->image_path)
                                    <img src="{{ asset('storage/' . $news->image_path) }}" class="w-12 h-12 rounded-xl object-cover border border-[#E2E8F0]">
                                @else
                                    <div class="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 text-xs font-medium border border-[#E2E8F0]">
                            ...
                            <td class="px-5 py-4 max-w-sm">
                                <a href="{{ route('public.news.show', $news->slug) }}" target="_blank" class="font-medium text-[#0F172A] hover:text-[#0F4C3A] transition-colors line-clamp-2">
                                    {{ $news->title }}
                                </a>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-right space-x-1.5">
                                <a href="{{ route('admin.news.edit', $news->id) }}" class="inline-flex items-center gap-1 bg-white hover:bg-slate-50 text-slate-700 border border-[#E2E8F0] hover:border-[#0F4C3A]/30 text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-colors">
                                    <svg class="w-3.5 h-3.5 text-[#0F4C3A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                    <span>Edit</span>
                                </a>
                                <form action="{{ route('admin.news.destroy', $news->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?')">
                                    @csrf
                                    @method('DELETE')
