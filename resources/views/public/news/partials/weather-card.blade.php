...
<aside aria-label="Informasi Cuaca Desa Catur BMKG" 
    ...
    <div class="absolute -bottom-10 -left-10 w-28 h-28 {{ $isDay ? 'bg-sky-400/15' : 'bg-blue-600/15' }} rounded-full blur-2xl pointer-events-none"></div>
    ...
    <div class="relative z-10 space-y-4">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="text-white/80 hover:text-white transition-colors shrink-0" 
                ...
                <svg class="w-4 h-4 sm:w-[18px] sm:h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
        ...
        @if($isAvailable)
            ...
            @if(!empty($forecastList) && count($forecastList) > 0)
                <div class="space-y-2 pt-1">
                    <div class="flex items-center justify-between text-[10px] uppercase tracking-wider text-sky-200/70 font-semibold px-0.5">
                    ...
                    <div class="flex items-center justify-between gap-1 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
                        ...
                        @foreach($forecastList as $slot)
                            <div class="flex flex-col items-center justify-center py-2 px-1.5 rounded-lg text-center flex-1 transition-all {{ $slot['is_current'] ? 'bg-white/20 backdrop-blur-xs font-bold' : 'hover:bg-white/10' }}">
            ...
            <div class="bg-white/[0.08] backdrop-blur-xs rounded-lg py-2 px-1 flex items-center justify-around divide-x divide-white/10 text-center">
                <!-- Kelembapan -->
    ...
    <div class="relative z-10 pt-3 flex items-center justify-between text-[10px] text-white/60">
        <span>
            ...
            <a href="{{ $weather['source_url'] ?? 'https://data.bmkg.go.id/prakiraan-cuaca/' }}" 
               target="_blank" 
