...
<header class="bg-white pt-8 sm:pt-10 pb-4 sm:pb-6 text-center max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <p class="text-xs sm:text-sm text-[#6C7B72] mt-2 font-medium">
        Pemerintah Desa Catur, Kecamatan Sambi, Kabupaten Boyolali
    </p>
</header>
        ...
        @if(isset($officials) && $officials->count() > 0)
            ...
            <div class="grid grid-cols-2 gap-3 sm:gap-4 md:hidden">
                @foreach($officials as $official)
                    <div class="bg-slate-50/80 rounded-xl p-2.5 sm:p-3.5 border border-slate-200/90 shadow-sm flex flex-col justify-between group hover:border-[#0A3D29]/40 hover:bg-white transition-all duration-200">
                        <div>
                            ...
                            <div class="relative rounded-lg overflow-hidden aspect-[3/4] w-full bg-slate-100 border border-slate-200/70 mb-2.5 flex items-center justify-center shadow-xs"
                                ...
                                @if(!empty($official->photo_path))
                                    <div x-show="!loaded && !error" class="absolute inset-0 animate-shimmer-glow z-10 pointer-events-none"></div>
                                    <img x-ref="mImg"
                                    ...
                                    <div x-show="error" class="absolute inset-0 flex items-center justify-center bg-slate-100 text-[#145C3B]/60">
                            ...
                            <!-- Nama -->
                            <h3 class="font-['Public_Sans',sans-serif] text-xs sm:text-sm font-extrabold text-slate-800 leading-snug line-clamp-2 group-hover:text-[#0A3D29] transition-colors">
                                {{ $official->name }}
                            </h3>
                        </div>
                    </div>
            ...
            <div class="hidden md:block space-y-6 lg:space-y-8">
                ...
                @if($headOfficial)
                    ...
                    <div class="bg-white rounded-xl p-5 lg:p-7 border border-slate-200/90 hover:border-[#0A3D29]/40 hover:bg-slate-50/50 shadow-sm hover:shadow-md transition-all duration-300 flex flex-row items-center gap-6 lg:gap-8 group">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <h2 class="font-['Public_Sans',sans-serif] text-2xl lg:text-3xl font-extrabold text-slate-800 leading-snug group-hover:text-[#0A3D29] transition-colors">
                                {{ $headName }}
                            </h2>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
                    @foreach($otherOfficials as $official)
                        <div class="bg-white rounded-xl p-3.5 sm:p-4 border border-slate-200/90 hover:border-[#0A3D29]/40 hover:bg-slate-50/50 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between group">
                            <div>
