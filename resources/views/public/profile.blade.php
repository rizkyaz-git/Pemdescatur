            ...
            <aside class="lg:col-span-3 space-y-6 order-2 lg:order-1">
                ...
                <div class="bg-white border border-[#DCE6DA] rounded-lg p-5 shadow-xs space-y-3.5">
                    ...
                    <div class="w-full max-w-[210px] aspect-[4/5] bg-[#EAF1E8]/40 overflow-hidden border border-[#DCE6DA] rounded-md">
                        ...
                        @else
                            <div class="w-full h-full flex flex-col items-center justify-center p-4 text-center bg-white text-slate-400">
                                @if(file_exists(public_path('images/logo_catur.png')))
                                    <img src="{{ asset('images/logo_catur.png') }}" alt="Logo Pemdes Catur" class="w-20 h-20 object-contain mb-2">
                                @else
                    ...
                    <div class="space-y-1.5 text-xs text-[#4B5851] leading-relaxed pt-1">
                        ...
                        <p class="leading-relaxed pt-1 text-[#4B5851]">
                            Memimpin pembangunan Desa Catur dengan komitmen penguatan pertanian padi organik, optimalisasi Desa Wisata, dan transparansi pelayanan berbasis Desa Cerdas Kemendes.
                        </p>
                    </div>
                ...
                <div class="bg-white border border-[#DCE6DA] rounded-lg p-5 shadow-xs space-y-3">
                    ...
                    <div class="space-y-2 text-xs text-[#4B5851]">
                        <div class="flex justify-between items-center py-1 border-b border-[#DCE6DA]/50">
                            ...
                            <span class="font-bold text-[#20332A]">{{ number_format($profile->luas_wilayah_ha ?? 244.5, 1) }} Ha</span>
                        </div>
                        <div class="flex justify-between items-center py-1 border-b border-[#DCE6DA]/50">
                        ...
                        <div class="flex justify-between items-center py-1 border-b border-[#DCE6DA]/50">
                            ...
                            <span class="font-bold text-[#20332A]">{{ number_format($profile->curah_hujan_mm ?? 2368) }} mm/th</span>
                        </div>
                        <div class="flex justify-between items-center py-1">
                            <span class="text-[#6C7B72]">Sumber Air</span>
                            <span class="font-bold text-[#20332A]">Waduk Wonotoro</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="prose prose-slate max-w-none text-[#20332A] text-sm sm:text-base leading-relaxed space-y-4 pt-1 font-sans prose-headings:font-serif prose-headings:text-[#20332A] prose-headings:tracking-tight prose-p:text-[#3E4D45] prose-p:leading-relaxed prose-blockquote:border-l-2 prose-blockquote:border-[#0A3D29] prose-blockquote:bg-[#EAF1E8]/30 prose-blockquote:py-2 prose-blockquote:px-4 prose-blockquote:italic prose-blockquote:font-serif prose-blockquote:rounded-r-sm prose-a:text-[#0A3D29] prose-a:font-semibold hover:prose-a:underline">
                    ...
                    @else
                        <p class="leading-relaxed text-[#3E4D45]">
                            Desa Catur merupakan salah satu desa di Kecamatan Sambi, Kabupaten Boyolali, Provinsi Jawa Tengah yang membawahi 13 pedukuhan. Sejak dahulu Desa Catur terkenal dengan keteladanan sektor pertanian padi organik yang didukung sistem irigasi stabil dari Waduk Wonotoro sepanjang tahun.
                        </p>
                    @endif
            ...
            <aside class="lg:col-span-3 space-y-6 order-3">
                ...
                <div class="bg-white border border-[#DCE6DA] rounded-lg p-5 shadow-xs space-y-3">
                    <div class="flex items-center justify-between border-b border-[#DCE6DA] pb-2">
                        ...
                        <a href="https://maps.app.goo.gl/E4SedfywjCLU7X8E6" target="_blank" rel="noopener noreferrer" class="text-[11px] font-bold text-[#0A3D29] hover:underline">
                            Maps ↗
                        </a>
                    </div>

                    <div class="space-y-1.5 text-xs text-[#4B5851]">
                        <p><span class="text-[#6C7B72] font-bold uppercase text-[10px]">Utara:</span> <span class="text-[#20332A] font-medium">{{ $profile->batas_utara ?? 'Desa Ngaglik' }}</span></p>
                        <p><span class="text-[#6C7B72] font-bold uppercase text-[10px]">Timur:</span> <span class="text-[#20332A] font-medium">{{ $profile->batas_timur ?? 'Desa Tawengan' }}</span></p>
                        <p><span class="text-[#6C7B72] font-bold uppercase text-[10px]">Selatan:</span> <span class="text-[#20332A] font-medium">{{ $profile->batas_selatan ?? 'Desa Glintang' }}</span></p>
                    ...
                    <div class="w-full h-36 overflow-hidden border border-[#DCE6DA] rounded-md mt-2">
                        <iframe
                            ...
                            loading="lazy"
                            title="Peta Satelit Wilayah Desa Catur"
                            allowfullscreen>
                        </iframe>
                    </div>
                </div>
