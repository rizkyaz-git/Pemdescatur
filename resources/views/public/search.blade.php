        ...
        <div class="space-y-6 max-w-3xl text-left">
            ...
            <form action="{{ url('/pencarian') }}" method="GET" class="relative max-w-2xl">
                <div class="relative flex items-center">
                    <input type="text" name="q" value="{{ $query }}" placeholder="Ketik kata kunci pencarian..."
                    ...
                    <button type="submit"
                        class="absolute right-1.5 bg-[#0A3D29] hover:bg-[#145C3B] text-white font-medium text-xs px-4 py-2 rounded-lg transition-colors">
                        Cari
                    </button>
                </div>
            </form>
                    ...
                    @foreach($results as $item)
                        <article class="py-6 first:pt-0 last:pb-0 group">
                            <a href="{{ $item['url'] }}" class="block space-y-2">
                                ...
                                <h2
                                    class="font-serif text-lg sm:text-xl font-bold text-slate-900 group-hover:text-[#0A3D29] transition-colors leading-snug">
                                    {{ $item['title'] }}
                                </h2>
                                ...
                                <div
                                    class="pt-1 flex items-center gap-1 text-xs font-medium text-slate-400 group-hover:text-[#0A3D29] transition-colors">
            ...
            @elseif(!empty($query))
                ...
                <div class="py-16 text-center max-w-md mx-auto space-y-3">
                    ...
                    <p class="text-xs sm:text-sm font-light text-slate-500 leading-relaxed">
                        Tidak ditemukan informasi yang sesuai dengan kata kunci <span
                            class="font-medium text-slate-800">“{{ $query }}”</span>. Silakan periksa kembali ejaan atau gunakan
                        kata kunci umum lainnya.
                    </p>
                    <div class="pt-2">
                        <a href="{{ route('home') }}"
                            class="inline-flex items-center gap-1.5 text-xs font-semibold text-[#0A3D29] hover:underline">
